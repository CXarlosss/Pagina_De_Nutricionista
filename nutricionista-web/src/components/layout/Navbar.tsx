...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-[36px] left-0 w-full z-50 backdrop-blur-lg bg-white/95 shadow-xl rounded-b-xl transition-colors duration-300"> {/* Eliminado dark:bg-zinc-900/95 */}
        ...
        <ul className="hidden md:flex gap-8 items-center">
          {navItems.map((item) => (
            ...
            >
              {item.submenu ? (
                <>
                  <button className="group relative text-lg font-semibold text-gray-800 hover:text-primary transition-all hover:scale-105 after:content-[''] after:absolute after:-bottom-1 after:left-0 after:h-[2px] after:w-0 group-hover:after:w-full after:bg-primary after:transition-all after:duration-300"> {/* Eliminado dark:text-gray-200 */}
                    {item.name}
                  </button>
                  ...
                  <AnimatePresence>
                    {openMenu === item.name && (
                      <motion.ul
                        ...
                        className="absolute top-full mt-2 bg-white/95 shadow-xl rounded-xl p-2 w-56 space-y-1 border border-gray-200 backdrop-blur-sm" // Eliminado dark:bg-neutral-800/95 y dark:border-zinc-700
                      >
